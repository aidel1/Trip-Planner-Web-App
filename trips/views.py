...
from .models import Trip, Hotel, Restaurant, Reservation
from datetime import datetime
from datetime import timedelta
from .static_activity_data import activities as all_activities
...
def generate_itinerary(selected_names, days):
    selected = [a for a in all_activities if a['name'] in selected_names]
    itinerary = {}
    ...
    for day in range(1, days + 1):
        ...
        for _ in range(2):  
            if idx < len(selected):
                itinerary[f"Day {day}"].append(selected[idx])
                idx += 1
            else:
                itinerary[f"Day {day}"].append({
                    "name": "Free Time",
                    "description": "Explore at your leisure",
                    "cost": 0
                })
    return itinerary
...
@login_required
def itinerary_view(request, *args):
    trip_details = request.session.get('trip_details')
    ...
    detailed_itinerary = generate_itinerary(selected_activities, num_days)
    ...
    activity_cost = sum(float(act.get("cost", 0)) * group_size for day in detailed_itinerary.values() for act in day)
